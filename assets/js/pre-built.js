    ...
    const textEl = document.getElementById('pcText');
    ...
    let focused = 0;
    document.querySelector('.pc-nav').addEventListener('keydown', (e) => {
        if (e.key === 'ArrowRight') focused = (focused + 1) % buttons.length;
        if (e.key === 'ArrowLeft') focused = (focused - 1 + buttons.length) % buttons.length;
	...
	document.getElementById('Build-Now').addEventListener('click', () => {
